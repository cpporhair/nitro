...
- 恢复模型：`scan tree leaf records + replay surviving WAL refs + rebuild logical winners + rebuild clean tree + reconstruct value runtime state + clean start`
...
client
  ...
  -> tree_guard-protected tree lookup
  -> value read
```

这里最重要的结构关系是：

1. 前台写先进入 `WAL + memtable`，然后就可以提交。
...
1. **提交 / publish**
   - 含义：某个 batch 从“已写入 WAL + memtable，但尚未对外可见”变成“对读者可见”。
...
3. 只要盘面格式可解析，且 Data Area 中的 leaf records、surviving WAL refs 与其中携带的 stable `value_ref` 足以恢复逻辑 KV 状态，旧 tree 结构差额就由 rebuild clean tree 吸收。
...
6. `durable_lsn = X` 表示当前 catalog 已经发布到 `X` 这个**连续前缀边界**；publish 可以一次跨过多个已经 ready 的 batch，但绝不能跳过更早的未 ready `batch_lsn`。
7. v1 不定义“分配了 `batch_lsn`，但 clean abort 后留下永久 hole 并继续服务”的运行态语义。一个 batch 一旦拿到 `batch_lsn`，它要么最终进入已发布前缀，要么运行期终止并由 recovery 处理其部分持久化痕迹。
...
2. 如果该 batch 在某次 seal 之前完成 front-side fan-out，则它在所有目标 front schedulers 上都写入 seal 前的 `A*`，随后这些 `A*` 一起变成 `F*`。
3. 如果该 batch 在 `CAT1` 安装完成之后才开始 front-side fan-out，则它在所有目标 front schedulers 上都写入 seal 后的新 `N*`。
...
`publish(batch_lsn)` 只能在“当前 catalog 的 `prs` 已覆盖该 batch 全部落点”时推进。

它推进的不是“某个离散 batch 已完成”的集合标记，而是：

```text
当前 ready 批次集合对应的最大连续 batch_lsn 前缀
```

也就是说：

1. 若 `batch_lsn = X + 1` 已 ready，但更早的 `X` 还没 ready，则 `durable_lsn` 仍停在旧值，不能跳号。
2. 一旦较早 hole 被补齐，publish 可以把 `durable_lsn` 一次推进到新的最大连续前缀末端。
3. 因为 v1 的 `batch_lsn` 分配语义本身是 gap-free 的，所以详细设计不需要额外定义“跳过永久缺号”的发布协议。
...
4. 只有 memtable 全 miss，才用 cat->prs.tree_guard.manifest 查 tree
5. 如命中 leaf record：
   - `kind = value`     -> 按 `value_ref` 读 value
...
9. winner 为 tombstone 时：
   ...
   - 如果 old tree-visible 状态已经是 tombstone，则只有 `tombstone.data_ver <= recovery_safe_lsn`、也就是所有更老旧版本都不可能再从 recovery 输入中出现时，后续 flush 才允许物理删除；否则本轮仍写 tombstone record。
...
2. 若某逻辑 key 的 old tree-visible 状态已经是 tombstone，而本轮仍没有更新的 PUT 覆盖它，则后续 flush 只有在 `tombstone.data_ver <= recovery_safe_lsn` 时，才允许把它物理删除。
...
4. tree 不以长期保留完整 tombstone 历史为目标，但也不能在第一次 DELETE flush 时就直接把 key 从 tree 中抹掉。
...
9. boot recovery 不依赖旧 runtime 的 `recovery_safe_lsn`；因此 winner 为 tombstone 时，clean tree 必须保守保留 tombstone。是否允许后续变成 absent，留给新 runtime 在重新建立 `recovery_safe_lsn` 后决定。
...
flush_round()
 ...
 >> maybe_root_change(
      on(tree_sched, update_superblock_async())
    )
```
...
recover()
 ...
 >> on(tree_sched, rebuild_clean_tree_and_manifest_reusing_winner_value_refs())
...
6. **DELETE 在 tree 中采用 `value -> tombstone -> absent` 的两阶段物化；tombstone 参与数据 winner 比较，但物理删除要等 `recovery_safe_lsn`。**
...
9. **恢复不重建旧 runtime frontier，而是 `scan tree leaf records + surviving WAL refs -> rebuild logical winners -> rebuild clean tree(reuse winner value_ref) -> reconstruct value runtime state -> clean start`。**
