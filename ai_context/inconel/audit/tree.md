...
### 2.2 读侧（`tree_lookup_sched`，RSM §4.7, RAP §4）

- `tree_lookup_state { lookup_id, node_cache, inflight_reads }` 每实例独立持有 readonly_frame_cache（tree_node domain）
- `home_tree_lookup(front_owner)` 稳定路由：由 front_owner 选定 home tree_lookup_sched，K 通常 < front_sched_count，按 NUMA 优先
...
| `reclaim()` handle | — | 缺失 |
| Data Area 碰撞检测 | — | 缺失 |
| `home_tree_lookup(front_owner)` 路由 | — | registry.hh L156-159 已显式标 "uncomment when corresponding scheduler exists"；当前 lookup() sender 让 caller 直接传 tree_sched 指针 |
...
### F7 — `home_tree_lookup_for_front` 路由缺失，`lookup()` sender 让 caller 直接传 sched 指针

**位置**：
- `core/registry.hh:148-159`（路由函数被注释掉，等待 front_sched）
- `tree/sender.hh:58-91`（lookup 接收 `lookup_scheduler_base*` 参数）

**问题**：spec RSM §4.7 严格要求 "same key → same tree_lookup_sched"（cache 局部性）。当前 lookup() 让 caller 传任意 sched 指针，没有 enforcement。

**当前评级**：因为没有 production caller（front_sched 不存在），现在还伤害不到任何人。但是：

- 等 front_sched / pipeline 落地时，写 caller 的人很容易随手传一个 "便当下手的 sched"，破坏 stable mapping
- 现在的 sender API 没有任何东西阻止这种情况
- registry.hh 里的注释 placeholder 是给 registry 的，不是给 sender 的

**Constraint A**：lookup() sender API 的当前 shape "看起来通用"，实际依赖 caller 自觉。不符合 fail-fast。

**修复方向**：把 lookup() sender 改成接收 `(key_range, manifest, front_owner)` 而不是 sched 指针；内部 `core::registry::home_tree_lookup_for_front(front_owner)` 解析。在 home_tree_lookup_for_front 上线前，sender 就已经按"通过 front_owner 解析"的形状写好——等 routing 函数落地直接打开。这样 caller 永远没机会偷工。
...
| `home_tree_lookup_for_front` routing | ✗ 缺失（F7） |
