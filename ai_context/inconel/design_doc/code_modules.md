...
B+ Tree 域。包含 `tree_sched`（单实例，状态拥有者）、`tree_lookup_sched`（K 实例，lookup/mapping）和 `tree_worker_sched`（K 实例，candidate materialization）。
...
| tree_lookup_sched | Leaf mapping | `keys_to_leaf_groups(flush_lookup_req)` → `flush_leaf_group_result[]` |
| tree_worker_sched | Candidate build | `build_leaf_candidates(flush_worker_req)` → `flush_candidate_batch` |
| tree_read_domain | Frame cache shard | `readonly_frame_cache`（tree_lookup_sched / tree_worker_sched 共享） |

**Owner 状态**：tree_sched `{ tree_allocator, checkpoint_guard, flush_state }`；tree_lookup_sched / tree_worker_sched `{ read_domain* }`；tree_read_domain `{ readonly_frame_cache }`
