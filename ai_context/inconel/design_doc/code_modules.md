...
| `tree_manifest` | `root_slot`, `slot_map` | tree(owns/构造), tree_lookup(只读消费) |
...
| `route_to_tree_lookup(front_owner)` | front → home tree_lookup_sched 稳定映射 |
...
| Frame Cache | readonly frame cache 抽象（tree_lookup_sched 和 value_alloc_sched 各自持有实例） |
...
B+ Tree 域。包含 tree_sched（单实例，状态拥有者）和 tree_lookup_sched（K 实例，无状态查询）。

| Scheduler | 职责 | handle |
|-----------|------|--------|
| tree_sched | Fold 算法 | 收集 memtable key → 定位受影响 leaf → NVMe 读 → merge |
| tree_sched | Tree slot 写入 | shadow slot 选择、consolidation（leaf/internal/root split） |
| tree_sched | Manifest 构造 | deep_copy old → 更新 slot_map → 替换 root（如变更） |
...
| tree_lookup_sched | Tree 查询 | `tree_lookup(key, manifest)` → `variant<leaf_value, leaf_tombstone, absent>` |
| tree_lookup_sched | Frame cache | 持有 tree_node `readonly_frame_cache` |

**Owner 状态**：tree_sched `{ tree_allocator, manifest, checkpoint_guard, flush_state }`；tree_lookup_sched `{ readonly_frame_cache }`

**设计文档**：`runtime_state_machine.md` §4，`flush_and_frontier_switch.md` §3/§9
...
| Sub-LBA 回收 | `freed_slots(page_base, class_idx, freed_mask)` |
| 整页回收 | `recycle_whole(class_idx, page_base)` |

**Owner 状态**：`value_alloc_state { bump_head, per-class pools, open_frames, dirty_pages, deferred_freed, readonly_frame_cache }`
...
| Flush | tree(check) → coord(capture_frontier) → fan-out front[](collect_gens) → reduce → tree(fold, write, build_manifest) → nvme(FLUSH) → tree(update_max_lsn) → coord(frontier_switch) → fan-out front[](release_gens) |
...
| 运行时部署参数 | front_sched_count, tree_lookup_sched_count, 核心绑定策略 |
