...
| TRIM drain | `drain_trim_once()` (`drain_trim_pending()` compatibility wrapper) |
...
配置、初始化、DB lifecycle 和 app root 边界。
...
| 生命周期 API | `start_db(db_options)(app_factory)` 拥有 device open、bootstrap format/recovery、scheduler build/start/destroy、maintenance stats；`stop_db()` 通过 root context 中的 `db_session_view` 请求停机 |
| 初始化流程 | 创建 real NVMe device → 运行 recovery（或 bootstrap format）→ 创建各 scheduler 实例 → 注册到 core/ 注册表 → 启动 share_nothing 主循环 |
| app root 边界 | `runtime/start_db.hh` 是唯一应用 root submit 边界；YCSB/其它 app 只返回 workload sender |
| 兼容入口 | `runtime/start.hh::start_runtime()` 仅保留为低层 scheduler bring-up helper，不是推荐 DB/app lifecycle 入口 |

**设计文档**：`design_overview.md` §3.3，§4.1；`plan/067_runtime_start_stop_api_design.md`
