...
输入：
  ...
  2. old_manifest: `frontier.old_guard->manifest`（当前 tree 结构 snapshot）
  3. old_tree_leaf_state: 通过 old_manifest 可以读取的旧 tree leaf records

输出：
  1. write_plan: 针对 memtable-touched keys 的 point mutations（upsert / logical delete）
...
// all_keys 只用于找出“本轮因前台更新而被触达的 leaf”
// 没有新 memtable 更新的旧 tombstone 不会单独进入 all_keys；
// 它们的物理删除统一放到 §3.4 的 leaf rewrite compact pass 中 opportunistically 处理。
...
for each logical_key in all_keys:
    ...
    assert(memtable_winner 存在)   // by construction: all_keys 来自 memtable keys
    ...
    if flush_winner.kind == value:
        ...
        if old_tree_record 存在且 old_tree_record.kind == value:
            ...
            // tombstone → tombstone：本轮仍先产出新的 tombstone winner
            // 是否进一步做 tombstone → absent，不在逐 key fold 里判断，
            // 而是在 §3.4 重写整页 leaf image 时统一做 page-local compact。
...
2. 对每个受影响的 leaf：
   ...
   b. 基于旧 leaf records + write_plan 中属于该 leaf 的 point mutations，生成 candidate leaf image
      - touched key：新增 / 更新 / 逻辑删除
      - untouched key：原样带入旧 leaf image
   c. 对 candidate leaf image 做 page-local compact pass：
      - 若 record.kind == tombstone && record.data_ver <= recovery_safe_lsn
        → 省略该 record（tombstone → absent）
      - 这是 opportunistic tombstone GC：
        只要该 leaf 因任一 key 更新而被重写，该页上其他已 safe 的 tombstone
        也可以顺手物理删除
      - v1 不为旧 tombstone 维护单独的全局 revisit / sweep 队列
   d. 若 compact 后 leaf image 与旧页完全一致 → 跳过该 leaf，零写入
   e. 如果合并后超出页容量 → split（产生新 leaf + 更新 parent internal）
   f. 如果合并后过于稀疏且相邻 leaf 也稀疏 → merge（可选优化，v1 可不做）
3. 写入新 leaf slot：
...
这一步对应 `coord_sched.handle_frontier_switch(old_guard, new_manifest, retired, flushed_gens_by_front)`。
在 `coord_sched` 上执行：
...
**但**：只有 root-change flush 会因为 superblock 未更新而阻塞 WAL reclaim。root-stable flush 不更新 superblock 仍然是安全的，因为 recovery 从相同的 root range base 出发可以扫描到最新 root slot。对应的统一规则见 `runtime_state_machine.md` §4.8。
...
flush_round()
  ...
  >> on(tree_sched, fold_visible_state(merged_gens, frontier.old_guard->manifest))
  ...
  >> on(coord_sched, frontier_switch(frontier.old_guard, new_manifest, retired, flushed_gens_by_front))
  >> for_each(front_sched_list)
     >> concurrent(N)
     >> flat_map([flushed_gens_by_front](auto* fs) {
            return fs->release_gens(flushed_gens_by_front[fs->owner_id]);
        })
     >> reduce()
