...
| correctness-carried hot data | `memtable_gen`（`kv_arena` 只承载 key bytes）、`memtable_entry`、`value_handle/value_ref` | `front_sched` | 否（只要仍被 memtable 触达） | 否 | 承载前台 live 索引；memtable hit 返回 value_ref |
...
2. memtable 的 key bytes 不是 page cache；它们住在 `memtable_gen.kv_arena` 里，和 gen 一起被 shared_ptr 整体保活。value bytes 不进入 memtable。
...
4. 真正可以按 cache 讨论的，只有 `readonly_frame_cache` 的 clean frame 驻留策略（tree_node 与 value_page 两个 domain）。memtable hit value 后也必须通过 value cache / `read_value(value_ref)` 取 body；value 层需要优先保留 recently-written / memtable-visible pages（见 `INC-055`）。
...
3. memtable 的 key bytes 住在 `memtable_gen.kv_arena` 里；value bytes 不在 memtable，`value_handle` 只携带 durable `value_ref`。跨核心读通过上面这条 pin 链保活的是 key bytes 与 entry metadata。
...
2. **不可被 cache eviction 破坏语义的 live metadata**：`memtable_gen.kv_arena`、`memtable_entry`、`value_handle/value_ref`（这些都随 gen 的 shared_ptr 整体生灭）。
...
1. **语义不同**
   ...
   - `memtable_gen.kv_arena` 被驱逐会破坏 memtable key/entry 语义（key bytes 消失）。
...
5. `memtable_gen.kv_arena` 默认也不进入 DMA 池；它承载 key bytes，不属于 frame/page。
...
### 9.3 memtable value hit 统一走 value cache

memtable hit 的语义：

1. 命中 memtable value 时，`lookup_memtable` 返回 durable `value_ref`，不返回 value bytes。
2. 调用方随后路由到 `value_alloc_sched.read_value(value_ref)` / `read_page_values(...)`；value owner 先查 dirty/resident/recently-written frame，再查 `readonly_frame_cache`，miss 才读 NVMe。
3. memtable 不保存 value body；`kv_arena` 只保存 key bytes。
4. 读性能目标由 value 层承担：FUA 完成后的 clean value page frame 应尽量保留到 memtable 可见窗口内，按 budget/epoch/pin 管理，避免 read-after-write 读盘（见 `INC-055`）。

因此，memtable 的 key bytes 不是任何 page cache 的一部分；value bytes 的内存驻留属于 value page cache / resident frame 策略。

### 9.4 value 模块的 cache 层次（冻结）

value 模块的 cache / residency 负责两类读：

1. tree hit 后的 `read_value(value_ref)` / `read_page_values(group)`；
2. memtable hit 后拿到 `value_ref` 的 read-after-write / recent-write 读。

当前冻结方向：

1. memtable 不保存 value body，所以 value 层必须承担 recently-written / memtable-visible value pages 的驻留优化。
2. 最低实现是 `value_page readonly_frame_cache` + `read_value()` / `read_page_values()` 的 copy-out 服务。
3. 为减少 memtable hit 后读盘，后续可在 value owner 内增加 dedicated write-through residency tier 或调整 FUA completion 后的 cache admission/eviction priority；该层仍必须受 value owner 的 budget/epoch/pin 约束，不能让 memtable 反向拥有 DMA frame。
4. 是否维护 `value_ref -> frame/slot` 的精细索引留给 `INC-055` 设计，但不能把 value body copy 回 memtable 作为替代。
...
2. hot data
   - memtable gens（含 `kv_arena` 承载 key bytes）、memtable entries
...
2. 不能为了省内存而从 live memtable gen 的 `kv_arena` 中局部释放 key bytes——arena 要么整块活着、要么随 gen 一起 sweep；value bytes 不在 memtable 中，由 value cache 的独立预算控制。
...
5. memtable 的 key bytes 住在 owning gen 的 `kv_arena` 里；memtable hit value 只返回 durable `value_ref`，由 `value_alloc_sched` 通过 value cache / NVMe 读取 body。
