...
| correctness-carried hot data | `memtable_gen`（含 `kv_arena` 承载 key/value bytes）、`memtable_entry`、`value_handle` | `front_sched` | 否（只要仍被 memtable 触达） | 否 | 承载前台 live 数据；memtable hit 必须直接返回 |
...
| cache 视图 | read-only frame cache | 按 domain 决定（tree_node 多 shard，每 `tree_read_domain` 一份；value_page 单 owner，由 `value_alloc_sched` 持有） | 是（仅限非 pinned clean 对象） | 否 | 降低读放大、避免重复 device read、支持原地解析/快速 copy-out |
...
2. memtable 的 key / value bytes 不是 page cache；它们住在 `memtable_gen.kv_arena` 里，和 gen 一起被 shared_ptr 整体保活，不是 "第二 owner 的 cache 视图"。
...
4. 真正可以按 cache 讨论的，只有 `readonly_frame_cache` 的 clean frame 驻留策略（tree_node 与 value_page 两个 domain）。v1 **不** 维护 `value_ref -> value bytes` 的 materialized 索引；memtable 的 value bytes 只在 owning gen 存活期间可从 `kv_arena` 直接读。
...
3. memtable 的 key / value bytes 都住在 `memtable_gen.kv_arena` 里；`value_handle.hot` 是 `value_view` 指向 arena 切片，不是独立 owner。跨核心读通过上面这条 pin 链间接保活，真正的 atomic gate 在 `std::shared_ptr<memtable_gen>` 的 control block——gen 活，arena 活，所有 key / value bytes 稳定。
...
2. **不可被 cache eviction 破坏语义的 live data**：`memtable_gen.kv_arena`、`memtable_entry`、`value_handle`（这些都随 gen 的 shared_ptr 整体生灭）。
...
1. **语义不同**
   ...
   - `memtable_gen.kv_arena` 被驱逐会破坏 memtable hit 语义（所有 key/value bytes 同时消失）。
...
- 对 multi-LBA value class，它是一个 segmented frame：逻辑上覆盖连续多个 LBA，物理 DMA payload 由多个独立 LBA-sized DMA pages 组成。
...
3. `span_lbas` 描述逻辑 LBA 覆盖范围；multi-LBA value extent 与单页对象使用同一套 frame 生命周期，但 payload 不要求 DMA-contiguous。
...
`value_page_frame` 比普通 `segmented_page_frame` 多出的不是“缓存逻辑”，而是 allocator 关心的页内可分配状态。当前实现已把 value round / prefill / read-miss / resident partial 页像放到 segmented LBA DMA frame 上；`value_page_frame` 的 class/slot 元数据扩展保留给后续更细的 frame-local free summary。

规则：

1. `free_bitmap/free_count` 只在 value page 域存在；当前 logical free-space truth 仍由 `value_space_manager` 持有，frame-local bitmap 只作为执行层 summary 扩展。
...
1. 真实 NVMe 生产路径上的 tree/value/superblock frame payload 都来自 LBA-sized DMA pages。
2. frame 描述符可以在普通内存里；只有 payload 必须是 DMA-resident。
3. tree node frame、value page frame、hole-fill / prefill page、tree flush writeback frame、superblock frame 都属于这条规则。
4. `publish_catalog`、`published_read_set`、`checkpoint_guard`、`tree_manifest`、memtable index、retire lists、`batch_cache` 不进入 DMA 池。
5. `memtable_gen.kv_arena` 默认也不进入 DMA 池；它承载 hot data，不属于 frame/page。
...
2. multi-LBA frame 在 v1 使用多个 LBA-sized DMA pages 组成 segmented frame；不要求 `span_lbas * lba_size` 的 DMA-contiguous class。
...
5. 对外主路径暴露的是“申请一个 LBA-sized DMA page / N 个 LBA pages”，不是“申请一个 tree page”或“申请一个 value page”。非常规大 scratch 才允许走单独 span 冷路径。

旧 `format::{read,write}_desc` 仍可作为 isolated legacy caller 的兼容桥：
real scheduler 会复制到 segmented LBA DMA frame 后再提交 PUMP NVMe。生产
tree/value/superblock runtime 不再依赖这层桥；它们直接构造
`memory::frame_read_desc` / `memory::frame_write_desc`，通过
`nvme::read_frame` / `nvme::write_frame` 提交多 LBA frame。
...
```text
alloc_lba_pages(count, numa_id):
1. 定位到 LBA-sized dma_pool_class
2. 先从 spdk_mempool_get(mp) 或 spdk_mempool_get_bulk(mp, ...) 获取 N 个独立 LBA pages
3. 若 mempool 不足：
   - 先触发 clean frame eviction / dirty flush
4. 若仍无法获得足够 LBA pages：
   - 进入既有 backpressure / 空间不足路径
```

multi-LBA frame 的构造是在这一步之上把 N 个 LBA pages 挂到同一个 `segmented_page_frame`；PUMP NVMe 仍通过 `get_pages` / `put_pages` 发出每 LBA 一个 page command。v1 不使用 SGL，也不要求这些 DMA pages 连续。

#### 归还路径

```text
free_lba_pages(pages, pool_class):
1. 仅当 frame 已不在任何 cache/view 中、没有 pin、不是 dirty、也不在 inflight writeback 中时，才允许归还
2. 用 spdk_mempool_put() / put_bulk() 归还每个 LBA page
```
...
现有 heap-buffer `page_frame` 只作为旧 cache policy compatibility carrier；真实 PUMP NVMe 生产路径使用 `segmented_page_frame`：每个逻辑 LBA 挂一个 `lba_dma_page`，multi-LBA tree/value/superblock frame 展开为多个 PUMP `page_concept` page，再由 `get_pages` / `put_pages` 提交。runtime frame I/O adapter 按同一 frame descriptor 语义逐 LBA 读写。
...
**2. value_alloc_sched 拥有 round/resident frames 并管理 DMA 页像**

`value_alloc_sched` 持有 round-local pages 和 `resident_partial_`（见 `runtime_state_machine.md` §6.3）。当 `persist_put_values` 内部需要打开一个 hole page 时：

1. 先检查 `value_alloc_sched` 本地的 `readonly_frame_cache` 是否命中该 page_base。
2. 若命中：直接从 cache 中取出 segmented frame，免去 device read。
3. 若未命中：通过本核 `nvme_sched` 用 `frame_read_desc` 读入完整页像。
4. 该 frame 成为本轮 `round_page`；commit 后若仍 partial，可进入 `resident_partial_` 供后续 PUT 复用。
...
| 对象 | Owner |
|------|-------|
| value round/resident page（dirty_append / dirty_hole_fill / resident partial） | `value_alloc_sched` |
...
**owner 规则**：所有 placement metadata（`whole_page_pool`、`hole_page_list`、`extent_free_pool`、`generic_free_spans`）和 round/resident frames 统一归 `value_alloc_sched` 所有（单一 owner）。`front_sched` 不参与 value 分配或持久化。

因此，value 写侧的两套状态都在 `value_alloc_sched` 上：

1. **non-resident free-space metadata**：告诉 allocator 位置和页内空洞分布。
2. **resident frame state**（round pages + `resident_partial_` + 本地 readonly_frame_cache）：告诉写路径当前代价和后续写法。
...
1. `value_alloc_sched`（value 读写唯一 owner）
   ...
   - 拥有 round-local / resident partial DMA frames
...
5. 当前 round/resident partial frame：代价最低，应优先继续填充或复用。
...
```text
1. 当前 resident_partial / round-local frame（只要仍有 free slot） ← value_alloc_sched 本地
...
以下流程完全在 `value_alloc_sched` 内部执行（`persist_put_values` → `space.allocate_batch` → `translate_claims_into_round`）：

```text
1. `value_space_manager` 返回 `nonresident_partial` claim。
2. 检查 `value_alloc_sched` 本地 `readonly_frame_cache` / `resident_partial` 是否命中该 `page_base`。
3. 若命中：直接取出 segmented frame；若未命中：从 LBA DMA pool 分配 segmented frame，并通过 `frame_read_desc` 读入完整页像。
4. frame 挂入本轮 `round_page`，写入 claim 指定的 byte offset。
5. 本轮结束时用 `frame_write_desc` + FUA 提交 writeback。
6. 若写后仍有 free slot：commit 后进入 `resident_partial` 并同步 `space.mark_cached_partial(...)`。
7. 若写满：转成 clean readonly 或释放，具体取决于 cache admission / residency budget。
```

关键点：

1. 一旦某个 partial page 已经成为 `resident_partial` 或本轮 `round_page`，后续可优先复用它。
...
旧文档中的 `slab_page_buf` 可以被看成 `value_alloc_sched` 的 round-local append frame：
...
### 8.6 实现注记 — 当前 round/resident frame 与 spec 概念的对应

当前 `apps/inconel/value/scheduler.hh` 把 §6.3 的 `open_frames[class_idx]` 目标模型工程化为两个执行层结构：

1. `round_page`：本轮 persist 正在写入或等待 FUA 的 segmented frame；它是当前 dirty ownership。
2. `resident_partial_`：commit 后仍有 free slot、由 scheduler 本地保留页像以便下一轮 persist 继续填充的 segmented frame。
...
- spec per-class 单一 `open_frame`：那要求每个 class 同时只有一帧在被 fill，所有 `dirty_append` / `dirty_hole_fill` 状态切换都集中在那一帧上；当前实现允许多个 partial page 以 `resident_partial_` 形式保留，是 spec 的一个超集，等到引入完整 frame state machine 时再收紧。

未来引入 frame state machine（`dirty_append` / `dirty_hole_fill` / `clean_readonly` 的显式状态、`hole_page_list` 的非 resident 描述符、每 class 单一 open frame）时，`resident_partial_` 与 round-local frames 会被收敛到更细的 per-class open-frame policy；底层 frame 仍保持 segmented LBA DMA page carrier。
...
3. owner 内部先查 dirty round frame / `resident_partial_`，再查本地 `readonly_frame_cache`。
...
### 9.3 memtable value bytes 独立于所有 page cache

memtable hit 的语义：

1. 命中 memtable value 时，`lookup_memtable` 直接返回 `winner.vh.hot`（已是 `value_view`，指向 owning `memtable_gen.kv_arena` 的某段切片）——zero-copy，调用方在 `read_handle` 作用域内直接读这段 bytes。
2. 不允许退化成对 `value_ref` 的 SSD 读。
3. memtable value bytes 的存在与 value page cache 的命中率完全独立。
4. value bytes 住在 owning `memtable_gen.kv_arena` 里，没有独立的 `hot_blob` 对象；跨核心读通过 `read_handle → cat → prs → front_read_set → std::shared_ptr<memtable_gen>` 的 pin 链间接保活，`std::shared_ptr<memtable_gen>` 的 control block 是整条链上唯一的跨线程 atomic gate。gen 活，arena 活，切片稳定。

因此，memtable 的 key/value bytes 不是任何 page cache 的一部分；它们属于 correctness-carried hot data，和 gen 一起生灭。

### 9.4 value 模块的 cache 层次（冻结）

value 模块只有 **一层** cache：`value_alloc_sched` 的 `readonly_frame_cache`（`value_page` domain），负责 resident page image 与 `read_value()` / `read_page_values()` 的 cache hit / copy-out。

v1 **不** 维护 `value_ref -> value bytes` 之类的 materialized value blob 索引。理由：

1. memtable 还活着时，对应 key 的 hot bytes 已经在 owning gen 的 `kv_arena` 里——读路径走 memtable hit，返回指向 arena 切片的 `value_view`，一次 copy 都不做。
2. memtable 被 flush、gen 被释放之后，同 key 后续读走 `tree_lookup → value_ref → value_alloc_sched.read_value()`；此时 value 所在页本就在 `value_page readonly_frame_cache` 里（写路径的 FUA write 完成后，frame 已经转为 `clean_readonly` 并入 cache），不需要再另起一层 materialized 索引。
3. 多维护一层 `value_ref -> value bytes` 索引，会迫使 value 存储带 refcount、跨 scheduler 协调，收益只有"memtable 刚释放后这一小段时间内的短命读"，代价远大于收益。

因此若其他文档里仍出现"`value_cache = value_ref -> bytes`"这种简写，应理解为**唯一的一层**：`value_page readonly_frame_cache` + `read_value()` / `read_page_values()` 的 copy-out 服务。没有"上层可选"这层。
...
2. `memtable_gen.kv_arena`
   - 承载该 gen 所有 key / value bytes，随 gen 析构一次 sweep 释放所有 chunk。
...
2. hot data
   - memtable gens（含 `kv_arena` 承载 key/value bytes）、memtable entries
...
2. 不能为了省内存而从 live memtable gen 的 `kv_arena` 中局部释放 key/value bytes——arena 要么整块活着、要么随 gen 一起 sweep。
...
- `value_alloc_sched` 的 round-local page（正在被写入）
- `value_alloc_sched` 的 `resident_partial_`（已 durable、仍保留页像以便复用）
...
| `memtable_gen.kv_arena` | correctness-carried hot data，不属于 page cache |
...
5. memtable 的 key/value bytes 住在 owning gen 的 `kv_arena` 里，独立于任何 page cache；memtable hit 绝不能退化成 SSD 读。
