...
`INC-031` / `INC-034` / `INC-036` 已分别由 step 016 / step 017 / step 018 收敛；当前本组暂无 open 项。
...
| INC-025 | value `writable_pages_` 是对 spec `hole_pages` + `open_frames` 双概念的简化（合成单一 per-class queue），但 spec ↔ 代码对应关系没文档化 | audit/value.md V6 | step 010 已完成：`value/scheduler.hh` 增补字段注释，`runtime_memory_and_cache.md` 新增实现注记，明确当前 `writable_pages_` 与 spec `hole_pages/open_frames` 的对应关系 |
