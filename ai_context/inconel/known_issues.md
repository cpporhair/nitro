...
`INC-044` / `INC-045` 已由 `INC-046` 写侧重写吞掉（优化的代码路径在重写后不存在）；当前本组暂无 open 项。
...
| INC-046 | **当前 tree 写侧整体不符合 Shadow CoW spec（design_overview §1.9 / FF §3.5），效果退化为普通 CoW + 含跨 round correctness bug，必须重写。** 这是 step 027/030/031/032/033 长链中一直没被审出来的核心 spec gap，所有 owner merge / cascade 路径相关的优化（含已废止的 INC-044 / INC-045）在不修这条之前都没有意义。两个具体偏离：(a) cascade 无条件走 root：`candidate_build.hh::initialize_cascade()` 把每个 affected leaf 的整条 parent chain 注册成 `internal_work`，不判断 child range_base 是否真的变化。spec §1.9 “普通更新不级联到 root” 完全失效，shadow CoW 摊销写放大 X/(X-1) 退化到普通 CoW 路径级联。(b) reformat 把 slot paddr 写进 internal page child_base：`owner_scheduler.hh::child_to_paddr` / `reformat_internal_node` 对 `unique_ptr` 分支用 `(*n)->new_paddr`，但 next slot 路径下 `new_paddr = slot_paddr(carrier, cur_slot+1) ≠ carrier`。`lookup_scheduler.hh::process_entries` 走 `manifest->resolve(child_base)` 假设 child_base 是 range_base，slot_map miss 直接 panic（`tree_manifest.hh::resolve`）。Round 1 没暴露纯属 slot 0 paddr 等于 range_base 的巧合；任意同 leaf 第二次 flush 后 lookup 必 panic。`INC-022` 描述（“缺 flush handle 整套基础设施”）与当前实际状态不符（写侧已落地一版但偏离 spec），重写后一并 supersede。 | 2026-04-19 INC-044 review 顺出 spec gap；`apps/inconel/tree/candidate_build.hh::initialize_cascade` / `apps/inconel/tree/owner_scheduler.hh::reformat_internal_node` + `child_to_paddr` / `apps/inconel/tree/lookup_scheduler.hh::process_entries` / `apps/inconel/core/tree_manifest.hh::resolve` | `urgent` (写侧 spec gap，含跨 round correctness bug + shadow CoW 摊销失效；不修这条，下面 owner merge / cascade / reclaim 路径相关的所有优化都没意义) | 重写写侧 cascade + reformat：(i) cascade short-circuit：worker 在 leaf 决定 next-slot/consolidation/split 之后，遇到 child range_base 不变就停止向上注册 `internal_work`；需要把 next-slot/consolidation 决策从 owner Phase 3 提前到 worker，或拆分 cascade 触发条件。(ii) `reformat_internal_node` / `child_to_paddr` 写 child_base 改用 `new_range_base` 而不是 `new_paddr`；passthrough child 已经是 range_base 不变。需要同步审 `mem_tree_node.new_range_base` 在所有 assign_paddr 入口正确填好（fresh range / next slot / consolidation 三条路径）。(iii) e2e harness 必须扩展到至少 2 round 同 leaf 重复写 + 跨 round lookup，覆盖 next slot child_base 路径，防止再退化。按 `feedback_no_premature_doc_update`：先开独立提案文档（不动主 design doc）写下死链路 + 新写侧设计，review 通过后再动 production 代码 |
