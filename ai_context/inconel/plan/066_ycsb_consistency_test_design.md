...
- `inconel_test_ycsb_concurrency_checker_e2e` 已覆盖 C1/C2/C3/C4/C5：put/read
...
- C5：aggressive auto-flush profile 下每个 write batch ACK 后逐 key
  ...
  `checker_maintenance.seal/flush/non_noop_flush > 0`。
- 尚未实现：C6-C8，以及跨 process history 的 recovery continuation
  checker。
...
```bash
apps/inconel/scripts/ycsb_consistency.sh [all|a0|a1|a2|a3|a4|a5|a6|a7|a8|a9|a10|c1|c2|c3|c4|c5]
```
...
`all` 执行 A0-A10 和当前已落地的 C1/C2/C3/C4/C5。单项场景适合调试或重跑失败项。
...
- `inconel_test_ycsb_concurrency_checker_e2e` 已实现 C1/C2/C3/C4/C5。
...
- C6-C8 尚未落地；frontier switch barrier 和 recovery continuation 仍是
  后续相邻项。
...
1. 需要把 read/write concurrency checker 从当前 C1/C2/C3/C4/C5 扩展到 C6-C8。
   现在已经能证明 put/read race 在 active、auto-seal 和 auto-flush 场景下
   的 interval 合法性，也能证明 delete/read/put race 下 tombstone 不穿透；
   batch ACK 后逐 key 可见性也已覆盖；还没有覆盖 frontier switch barrier
   和 recovery continuation。
