...
- `inconel_test_ycsb_concurrency_checker_e2e` 已覆盖 C1/C3：put/read
  interval checker 在 maintenance-off 和 aggressive auto-flush 两种 real
  NVMe 场景下通过，C3 观察到 non-noop flush。
...
当前已落地的 066C 入口是
`inconel_test_ycsb_concurrency_checker_e2e`，脚本场景为 `c1` 和 `c3`：
...
- 尚未实现：C2、C4-C8，以及跨 process history 的 recovery continuation
  checker。
...
```bash
apps/inconel/scripts/ycsb_consistency.sh [all|a0|a1|a2|a3|a4|a5|a6|a7|a8|a9|a10|c1|c3]
```
...
- 解析 phase / maintenance counters，要求所有 real run 的
  YCSB `maintenance.failed=0`；对 C3 要求
  `checker_maintenance.failed=0`，并按本节验收规则 fail-fast。

`all` 执行 A0-A10 和当前已落地的 C1/C3。单项场景适合调试或重跑失败项。
...
- `inconel_test_ycsb_concurrency_checker_e2e` 已实现 C1/C3。
...
- C2、C4-C8 尚未落地；delete/read/put race、batch ACK completeness、
  frontier switch barrier 和 recovery continuation 仍是后续相邻项。
...
1. 需要把 read/write concurrency checker 从当前 C1/C3 扩展到 C2、C4-C8。
   现在已经能证明 put/read race 在 active 和 auto-flush 场景下的 interval
   合法性，但还没有覆盖 DELETE、batch barrier、frontier switch barrier 和
