...
- 尚未实现：C5-C8，以及跨 process history 的 recovery continuation
  checker。
...
```bash
apps/inconel/scripts/ycsb_consistency.sh [all|a0|a1|a2|a3|a4|a5|a6|a7|a8|a9|a10|c1|c2|c3|c4]
```
...
- 解析 phase / maintenance counters，要求所有 real run 的
  YCSB `maintenance.failed=0`；对 C2/C3 要求
  `checker_maintenance.failed=0`，并按本节验收规则 fail-fast。

`all` 执行 A0-A10 和当前已落地的 C1/C2/C3/C4。单项场景适合调试或重跑失败项。
...
- `inconel_test_ycsb_concurrency_checker_e2e` 已实现 C1/C2/C3/C4。
...
- C5-C8 尚未落地；batch ACK completeness、frontier switch barrier 和
  recovery continuation 仍是后续相邻项。
...
1. 需要把 read/write concurrency checker 从当前 C1/C2/C3/C4 扩展到 C5-C8。
   现在已经能证明 put/read race 在 active、auto-seal 和 auto-flush 场景下
   的 interval 合法性，也能证明 delete/read/put race 下 tombstone 不穿透；
   还没有覆盖 batch barrier、frontier switch barrier 和 recovery
   continuation。
