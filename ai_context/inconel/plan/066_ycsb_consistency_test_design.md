...
```bash
apps/inconel/scripts/ycsb_consistency.sh [all|a0|a1|a2|a3|a4|a5|a6|a7|a8|a9|a10]
```

脚本职责：

- 构建 `inconel_ycsb` 和 `inconel_real_nvme_compile_check`。
...
- 解析 phase / maintenance counters，要求所有 real run 的
  `maintenance.failed=0`，并按本节验收规则 fail-fast。

`all` 执行 A0-A10。单项场景适合调试或重跑失败项。
...
1. 需要 read/write concurrency checker。没有 operation interval log，就不能
