...
| placement metadata | `whole_page_pool`、`hole_page_list`、`extent_free_pool`、allocator head | `value_alloc_sched` | 不适用 | 否 | 告诉 allocator 哪些空间可复用 |
| cache 视图 | read-only frame cache、可选 materialized value view index | 各 scheduler / 各核心 local shard | 是（仅限非 pinned clean 对象） | 否 | 降低读放大、避免重复 device read、支持 zero-copy |
...
3. `whole_page_pool / hole_page_list` 不是 cache；它们是 allocation metadata，归 `value_alloc_sched` 所有。
...
1. 读路径：`NVMe -> DMA frame -> parse / value_view`，不需要再复制到普通 heap page。
...
3. writeback 的直接数据源
4. `value_view` / zero-copy parse 所 pin 的底层内存
...
**owner 规则**：所有 placement metadata（`whole_page_pool`、`hole_page_list`、`extent_free_pool`）和 open frames 统一归 `value_alloc_sched` 所有（单一 owner）。`front_sched` 不参与 value 分配或持久化。
...
1. `value_alloc_sched`（value 写侧唯一 owner）
   - 拥有所有 value placement metadata：`whole_page_pool`、`hole_page_list`、`extent_free_pool`、allocator head
   - 拥有 per-class `open_frames`（dirty_append / dirty_hole_fill DMA frames）
   - 拥有本地 `readonly_frame_cache`（value_page domain，用于 hole reuse 时避免 device read）
   - 通过本核 `nvme_sched` 提交 value FUA 写入

2. `front_sched(owner)`
   - 拥有本 owner 的 WAL tail frame
   - 拥有本 owner 的 `readonly_frame_cache`（tree_node + value_page domain，用于读路径）
...
4. read cache local shard（各 scheduler 内部）
...
新模型下，这些元数据和 DMA 写入 ownership 统一归 `value_alloc_sched`（单线程，天然互斥）。`value_alloc_sched` 本地的 `readonly_frame_cache` 用于 hole page 打开前的 cache hit 优化（免去 device read），但 cache 不跟踪 free-slot 信息。各 `front_sched` 的 `readonly_frame_cache` 只用于读路径（tree lookup + value read），不参与 value 写入。

### 7.3 允许重复 residency，但不允许重复可写 ownership

同一个 clean page image 可以在多个 read cache shard 中各自驻留（例如 `value_alloc_sched` 和某个 `front_sched` 各自 cache 了同一个 value page），这只是空间换性能。

但：

1. dirty frame 的可写 ownership 只能有一份，且只在 `value_alloc_sched` 上（value pages）或 `front_sched` 上（WAL tail）。
2. value page 一旦被 `value_alloc_sched` 打开为 dirty（dirty_hole_fill / dirty_append），其他 sched 的旧 clean 副本只能保留为只读或自然淘汰，不影响 allocation。
3. `value_alloc_sched` 的 open frame 写满并 writeback 后转为 clean_readonly，可继续留在本地 cache；其他 sched 的读路径若需要该页，独立从 NVMe 读入各自 cache。
...
3. node frame miss 时从 NVMe 读入 frame
4. 解析在 frame 上原地进行
...
对 tree hit value，read path 应优先返回“对 resident frame 的 view”，而不是立即复制成新 heap buffer。

概念结构：

```cpp
struct value_view {
    frame_pin pin;                                   // 保活 DMA page
...
1. `value_view` 通过 `frame_pin` 保活 frame residency，不是 correctness owner。
2. 上层若需要 owning bytes，可以再显式 materialize。
3. 对 sub-LBA value，通常一个 frame 就够。
4. 对 multi-LBA value，可扩成 span frame 或多个 frame slices；不改变 API 抽象。
...
1. **底层**：page/frame cache，负责 resident page image 与 zero-copy `value_view`。
2. **上层可选**：materialized value view/blob 的复用索引。
...
1. lookup key 携带 runtime `reuse_epoch`
2. 在 page / extent 进入可复用状态前，对相关 cache shards 发 invalidate

本文冻结语义，不强制二选一的具体落法；但禁止只用裸 `paddr` 且没有任何复用失效机制。

### 10.3 Hole Page 相关 Frame 的回收

`value_alloc_sched` 或 `front_sched`（读路径）的 `readonly_frame_cache` 中都可能存在 partially-free value page 的 clean 页像。这类 frame 的回收遵循与普通 clean frame 相同的规则：

1. `pin_count == 0` 时可按 LRU 驱逐。
2. 驱逐只丢页像，不影响 `value_alloc_sched` 持有的 hole 元数据。
3. 若页已经完全死亡并被 `value_alloc_sched` 回收到 `whole_page_pool`，各 sched 的 cache 中该 frame 需按地址复用失效机制处理（同 Section 10.2）。
4. `value_alloc_sched` 上 `dirty_hole_fill` 状态的 frame 不能驱逐，只能在 writeback completion 后退出 dirty set。
...
| 旧术语 | 在本文中的统一解释 |
|--------|------------------|
| `node_cache` | `readonly_frame_cache` 在 tree domain 的逻辑视图 |
