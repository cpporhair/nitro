...
test_process_two_pass() {
    ...
    flush_leaf_group groups[] = {
        { make_paddr(1000), make_paddr(1000),
    ...
    CHECK(state.result.leaves.size() == 1);
    CHECK(state.result.leaves[0].record_count == 3);  // 10, 15, 20
...
test_process_cache_hit() {
    ...
    flush_leaf_group groups[] = {
        { make_paddr(2000), make_paddr(2000),
          std::span<const flush_key_group>(keys, 1) },
    };
    ...
    CHECK(state.all_done);
    CHECK(state.result.leaves.size() == 1);
    CHECK(state.result.leaves[0].record_count == 2);  // 10, 20
...
test_bounded_reads() {
    ...
    CHECK(state.all_done);
    CHECK(state.result.leaves.size() == 5);
