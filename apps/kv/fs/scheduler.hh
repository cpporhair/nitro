...
#ifndef APPS_KV_FS_SCHEDULER_HH
#define APPS_KV_FS_SCHEDULER_HH
...
namespace pump::core {
    ...
    struct
    compute_sender_type<context_t, apps::kv::fs::_allocate::sender<scheduler_t>> {
        ...
        get_value_type_identity() {
            return static_cast<apps::kv::fs::_allocate::leader_res *>(nullptr);
        }
    };

    template <typename context_t, typename scheduler_t>
    struct
    compute_sender_type<context_t, apps::kv::fs::_metadata::sender<scheduler_t>> {
        consteval static uint32_t
        count_value() {
            return 1;
        }
        ...
        consteval static auto
        get_value_type_identity() {
            return std::type_identity<apps::kv::fs::_metadata::leader_res *>{};
        }
    };

    template <typename context_t, typename scheduler_t>
    struct
    compute_sender_type<context_t, apps::kv::fs::_freepage::sender<scheduler_t>> {
        consteval static auto
        count_value() {
            return 0;
        }
