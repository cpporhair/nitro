...
#ifndef APPS_KV_START_DB_HH
#define APPS_KV_START_DB_HH
...
namespace apps::kv {
    ...
    inline auto
    init_proc_env() {
        return pump::sender::then([](){ runtime::init_proc_env(); });
    }

    inline auto
    init_nvme_env() {
        return pump::sender::then([](){ runtime::init_ssd(); });
    }

    inline auto
    save_index() {
        return pump::sender::then([](...){
    ...
    inline auto
    init_indexes() {
        return pump::sender::get_context<popl::option_parser*>()
            >> pump::sender::then([](popl::option_parser* c){
    ...
    inline auto
    run(func_t&& func) {
        return pump::sender::then(__fwd__(func));
    }

    inline auto
    finally() {
        return pump::sender::ignore_args()
            >> pump::sender::any_exception([](std::exception_ptr p){
                try {
    ...
    init_config() {
        return pump::sender::get_context<popl::option_parser*>()
            >> pump::sender::then([](popl::option_parser* op) mutable {
    ...
    with_options(popl::option_parser* op) {
        return [op](auto &&b) mutable {
            return pump::sender::with_context(op)(__fwd__(b));
        };
    }
